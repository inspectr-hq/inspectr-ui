  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <div>
          <label htmlFor="rule-name" className="text-sm font-medium text-gray-900 dark:text-gray-50">
            {isEditing ? 'Edit Rule' : 'Rule Name'}
          </label>
          <p className="text-xs text-gray-500 dark:text-gray-500">
            {isEditing ? 'Update the selected rule and save your changes.' : 'Define a descriptive name for your rule.'}
          </p>
        </div>
        {isEditing && (
      ...
      <div className="space-y-2">
        <label htmlFor="rule-description" className="text-sm font-medium text-gray-900 dark:text-gray-50">
          Description
        </label>
        <textarea
          id="rule-description"
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          <label htmlFor="rule-event" className="text-sm font-medium text-gray-900 dark:text-gray-50">
            Event Trigger
          </label>
          <select
            id="rule-event"
        ...
        <div className="grid gap-2 sm:justify-items-end">
          ...
          <div className="space-y-2">
            <label htmlFor="rule-priority" className="text-sm font-medium text-gray-900 dark:text-gray-50">
              Priority
            </label>
            <input
              id="rule-priority"
        ...
        <div className="space-y-6 px-4 py-5">
          <div className="rounded-lg border-l-4 border-orange-600 bg-orange-50/60 p-4 dark:border-orange-500 dark:bg-orange-500/10">
            <div className="flex items-center gap-4">
              <span className="flex aspect-square h-10 items-center justify-center rounded-lg bg-orange-600 dark:bg-orange-500" aria-hidden="true">
                <svg
              ...
              <div className="truncate">
                <h5 className="text-sm font-medium capitalize text-gray-900 dark:text-gray-50">Event</h5>
                <p className="text-xs text-gray-600 dark:text-gray-400">Select the operation event you want to monitor.</p>
              </div>
            </div>
            <div className="mt-4">
              <label className="text-xs font-medium text-gray-900 dark:text-gray-200" htmlFor="builder-event">
                Event
              </label>
              <select
                id="builder-event"
          ...
          <div className="space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-3">
                <label className="text-xs font-medium text-gray-500 dark:text-gray-400" htmlFor="expression-type">
                  Condition Logic
                </label>
                <select
                  id="expression-type"
            ...
            {form.conditions.map((condition, index) => (
              <div key={index} className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <div className="flex items-start gap-4 px-4 py-4">
                  <span className="flex aspect-square h-10 items-center justify-center rounded-lg bg-sky-500" aria-hidden="true">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
                  ...
                  <div className="flex-1 space-y-3">
                    <div className="grid gap-3 sm:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
                      ...
                      <div className="grid gap-3 sm:grid-cols-2">
                        <div className="space-y-1">
                          <label className="text-xs font-medium text-gray-500 dark:text-gray-400">Operator</label>
                          <select
                        ...
                        <div className="space-y-1">
                          <label className="text-xs font-medium text-gray-500 dark:text-gray-400">Value Type</label>
                          <select
                            value={condition.valueType}
                            onChange={(event) => onConditionChange(index, 'valueType', event.target.value)}
                            className="block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm transition focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-50 dark:focus:border-blue-700 dark:focus:ring-blue-700/30"
                          >
                    ...
                    <div className="grid gap-3 sm:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
                      <div className="space-y-1">
                        <label className="text-xs font-medium text-gray-500 dark:text-gray-400">Compare value</label>
                        {condition.valueType === 'boolean' ? (
                          ...
                          <input
                            ...
                            onChange={(event) => onConditionChange(index, 'value', event.target.value)}
                            placeholder={condition.valueType === 'number' ? 'e.g. 200' : 'e.g. /api/payments'}
                            className="block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm transition focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-50 dark:focus:border-blue-700 dark:focus:ring-blue-700/30"
                          />
                        )}
                      </div>
          ...
          <div className="space-y-4">
            ...
            {form.actions.map((action) => {
              const definition = actionsCatalog.find((item) => item.type === action.type) || { params: [] };
              return (
                <div key={action.id} className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
                  <div className="flex items-start gap-4 px-4 py-4">
                    <span className="flex aspect-square h-10 items-center justify-center rounded-lg bg-emerald-500" aria-hidden="true">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="24"
                        height="24"
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                        <div className="flex-1 min-w-[180px] space-y-1">
                          ...
                          <select
                            ...
                            className="block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm transition focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-50 dark:focus:border-blue-700 dark:focus:ring-blue-700/30"
                          >
                            {actionsCatalog.length === 0 && <option value="">No actions available</option>}
                              ...
                              {isBoolean ? (
                                <div className="flex items-center gap-3">
                                  <input
                                    ...
                                    onChange={(event) => onActionParamChange(action.id, param.name, event.target.checked)}
                                    className="size-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-900"
                                  />
                                  <span className="text-xs text-gray-600 dark:text-gray-400">{param.description}</span>
                                </div>
                              ) : (
                                <>
                                  <input
                                    id={fieldId}
                                    type="text"
                                    value={typeof currentValue === 'string' ? currentValue : isArray ? (currentValue || []).join(', ') : currentValue ?? ''}
                                    onChange={(event) => onActionParamChange(action.id, param.name, event.target.value)}
                                    placeholder={isArray ? 'tag.one, tag.two' : ''}
                                    className="block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm transition focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-50 dark:focus:border-blue-700 dark:focus:ring-blue-700/30"
                                  />
                                  {param.description && (
                                    <p className="text-xs text-gray-500 dark:text-gray-500">{param.description}</p>
                                  )}
                                </>
                              )}
                            </div>
